...
// Relatório por PROCEDIMENTO - usa cidade_destino
router.get('/relatorio', function(req, res) {
    try {
        ...
        var sql = "SELECT d.*, p.nome, p.documento_tipo, p.documento_valor, p.cidade as cidade_origem, p.telefone FROM demandas d INNER JOIN pacientes p ON p.id = d.paciente_id WHERE 1=1";
        var params = [];
        ...
        // Filtro de cidade agora busca em cidade_destino (para onde saiu o atendimento)
        if (cidade) { sql += ' AND d.cidade_destino=?'; params.push(cidade); }
        if (status) { sql += ' AND d.status=?'; params.push(status); }
        if (sistema) { sql += ' AND d.sistema=?'; params.push(sistema); }
        sql += ' ORDER BY d.data_entrada DESC';
